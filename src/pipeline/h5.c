...
#else
    /* Uncompressed direct chunk write — still faster than H5Dwrite+hyperslab
     * because it bypasses the filter pipeline and chunk cache. */
    if (H5Dwrite_chunk(sink->dset, H5P_DEFAULT, 0, offset,
                       n_elem * sizeof(uint16_t), pixels) < 0) {
        fprintf(stderr, "h5_sink_write_frame: H5Dwrite_chunk failed at frame %u\n",
                sink->frame_idx);
        sink->errored = 1;
        return -1;
